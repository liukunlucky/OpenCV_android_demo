...
/**
 * Created by Lucky on 2018/6/22.
 */
public class Show_images2 extends Activity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Uri two=intent2.getParcelableExtra("com.example.vanishingpoint.two");
        try {
            bitmap = BitmapFactory.decodeStream(getContentResolver().openInputStream(two));
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
            copyBitmap = Bitmap.createBitmap(640,360,Bitmap.Config.RGB_565);
            ImgView1.setImageBitmap(bitmap);
            ImgView1.setRotation(90);
        Log.d("LKLKLKLKLKLK",String.valueOf(bitmap.getWidth())+","+String.valueOf(bitmap.getHeight()));
        //定义“消失点”按钮的动作
    ...
    private void vanishingpoint(){
        ...
        Imgproc.resize(srcImg,outImg,new Size(640,360));
        Log.d("LKLKLKLKLK",outImg.size().toString());
        Imgproc.cvtColor(outImg,thresholdImage,Imgproc.COLOR_BGR2GRAY,4);
        Imgproc.cvtColor(outImg,dstMat,Imgproc.COLOR_BGR2RGB,4);
        Imgproc.Canny(thresholdImage,thresholdImage,30,70,3,false);
        int row=outImg.rows();
        int col=outImg.cols();
        Mat copyImg=outImg.clone();
        tw.setText("图像的宽度为："+row+" Pixels"+"\n");
        tw.append("图像的高度为："+col+" Pixels"+"\n");
        /*  这里是对Mat的读取和写入进行一个测试
        Mat dst=new Mat(1,2, CvType.CV_32FC2);
        double tt[]={7.5,11.0};
        double tt2[]={10.5,34.0};
        dst.put(0,0,tt);
        dst.put(0,1,tt2);
        Log.d("刘坤刘坤lklklklk",dst.size().toString());
        double value[]=dst.get(0,1);
        Log.d("刘坤刘坤lklklklk",String.valueOf(value[1]));
        //dst.put(1,1,1);
        //Log.d("刘坤刘坤lklklklk",dst.size().toString());
        */
        Mat cdst1=new Mat();Mat cdst2=new Mat();
        ...
        //Log.d("line的type：",String.valueOf(lines.type()));
        //tw.append("lines.size"+lines.size()+"\n");
        //摘自csdn.com***************************
